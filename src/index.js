...
async function fetchStockData() {
    ...
    try {
        const stockData = await Promise.all(tickersArr.map(async (ticker) => {
            //const url = `https://api.polygon.io/v2/aggs/ticker/${ticker}/range/1/day/${dates.startDate}/${dates.endDate}?apiKey=ZQgz7pKxxG4Y0pe3fts2IT_fbZ5kjzUN`
			const url = "https://polygon-api-worker.moondog.workers.dev/"
			// Make a fetch request to the Worker
	// Make a fetch request to the Worker
			// send query parameters (ticker, start and end dates) on the URL
			...
			const response = await fetch(`${url}?${params.toString()}`, {
				method: 'GET' // change to 'POST' and include a body if your worker expects POST requests
			});
...
async function fetchReport(data) {
    ...
    try {
		const url = "https://open-api-worker.moondog.workers.dev/"
		// Make a fetch request to the Worker
		const response = await fetch(url, {
			method: 'POST',
			headers: {
